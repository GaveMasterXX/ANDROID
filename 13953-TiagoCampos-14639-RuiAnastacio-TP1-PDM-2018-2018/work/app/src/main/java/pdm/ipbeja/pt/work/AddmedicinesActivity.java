...
public class AddmedicinesActivity extends AppCompatActivity implements TimePickerDialog.OnTimeSetListener, OnClickListener {
    ...
    public void onClickAddMedicine(View view) {
       ...
       String typeMed = "capsule";
        String[] splitHour;
        int hourClock = 00;
        int minutesClock = 00;
        medicineName = medicineNameText.getText().toString(); //nome do medicamento

        HealthBoxDatabase.getInstance(this).medicinesDao().insert(new Medicines(0, medicineName, typeMed,day, monthMed, year, daysTake)); ///faz o inser na base de dados
        List<Medicines> medicinesList = HealthBoxDatabase.getInstance(this).medicinesDao().getMedicines(); //devolve a lista de medicamentos4

       // for (int i = 0; i < medicinesList.size(); i++) { //percorre a lista de medicamentos da db para achar o medicamento inserido
            String name = medicinesList.get(medicinesList.size() - 1).getName().toString();
            if( name.equals(medicineName)){
                int idMed = (int) medicinesList.get(medicinesList.size() - 1).getIdMed();
                for (int j = 0; j < numberselect; j++) {
                    String hours = myTextViews[j].getText().toString();
                    HealthBoxDatabase.getInstance(this).historicLogDao().insert(new HistoricLog(0,idMed, hours, day, monthMed, year, "Take")); //inser na tabela historicLog
                    List<HistoricLog> list = HealthBoxDatabase.getInstance(this).historicLogDao().getAllHistoricLogs();
                    splitHour = hours.split(":");
                    ...
                    for (int k = list.size() - 1; k > list.size()-(numberselect + 1); k--) {
                                long identifier = list.get(k).getIdLog();
